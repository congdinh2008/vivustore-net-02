namespace ViVuStore.MVC.Services.BaseServices;
...
public class BaseService<T> : IBaseService<T> where T : BaseEntity
{
    ...
    public virtual IEnumerable<T> GetAll(bool isIncludeDeleted = false)
    {
        if (!isIncludeDeleted)
        {
            return _unitOfWork.GenericRepository<T>().GetQuery(x => x.DeletedAt == null).ToList();
        }
    ...
    public virtual async Task<IEnumerable<T>> GetAllAsync(bool isIncludeDeleted = false)
    {
        if (!isIncludeDeleted)
        {
            return await _unitOfWork.GenericRepository<T>().GetQuery(x => x.DeletedAt == null).ToListAsync();
        }
